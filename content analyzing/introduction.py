...
def introduction_questions(speech):
    ...
    for i in range(len(tokens)):
        if (tokens[i].pos_ == 'AUX' and tokens[i + 1].pos_ == 'PRON' and tokens[i + 2].pos_ == 'VERB'):
            identified_questions.append(f"{tokens[i]} {tokens[i + 1]} {tokens[i + 2]}")

    for i in range(len(tokens)):
        if (tokens[i].pos_ == 'ADV' and tokens[i + 1].pos_ == 'VERB' and tokens[i + 2].pos_ == 'PRON'):
            identified_questions.append(f"{tokens[i]} {tokens[i + 1]} {tokens[i + 2]}")

    for i in range(len(tokens)):
        if (tokens[i].pos_ == 'ADV' and tokens[i + 1].pos_ == 'AUX' and tokens[i + 2].pos_ == 'DET'):
            identified_questions.append(f"{tokens[i]} {tokens[i + 1]} {tokens[i + 2]}")
